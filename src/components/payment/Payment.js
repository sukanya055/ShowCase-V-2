...
const Payment = () => {
    ...
    const f = async(amount) =>{
        ...
        try{
            const verifyUrl = "http://3.110.147.43:5000/api/payment/changerole";
	...
	const initPayment = (data, pay) => {
		const options = {
			...
			handler: async (response) => {
				try {
					const verifyUrl = "http://3.110.147.43:5000/api/payment/verify";
    ...
    const handleSubmit = async(e) => {
        e.preventDefault();
        try{
            const orderurl = "http://3.110.147.43:5000/api/payment/orders";
