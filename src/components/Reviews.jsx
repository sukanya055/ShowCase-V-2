  ...
  return (
    <div className="flex flex-col gap-8 px-5 py-2  shadow-md  rounded-lg ">
      <div className="flex justify-between border-b-[1px] pb-2 border-gray-400 items-center">
        ...
        <select className="select bg-transparent  max-w-xs">
          <option disabled selected>
            Sort By
          </option>

          <option>Oldest one</option>
        ...
        {comments.map((comment) => (
          <div className="flex flex-col gap-2">
                ...
                <div className="rating rating-sm">
                  {[1, 2, 3, 4, 5].map((item) => (
                    <input
                      ...
                      checked={item === comment.ratings ? true : false}
                      readOnly
                    />
                  ))}
                </div>
            ...
            <h2 className="text-sm font-normal text-gray-700">
              By <span className="font-bold ">{comment.user}</span> 11 days ago
            </h2>
            <p className="text-md text-gray-600">{comment.desc}</p>
          </div>
        ))}
      </div>
    </div>
