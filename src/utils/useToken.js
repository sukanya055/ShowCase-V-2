      ...
      if (user) {
        try {
          const userexist = await fetch(
            "http://13.234.213.238:3000/exist",
            {
              method: "POST",
              headers: {
          ...
          let response;
          const data = await axios.post("http://13.234.213.238:3000/user/login", {
            email: user?.email,
            google: "google",
          });
          ...
          const roles = await fetch("http://13.234.213.238:3000/user/infor", {
            method: "GET",
            headers: {
